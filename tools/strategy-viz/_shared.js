// Shared browser helpers for strategy-viz HTML pages.
// Mirrors the Python helpers in _pricing.py / _specs.py / _common.py so a
// single page can render all three views without duplicating math.
//
// Loaded as a plain <script>; exposes a global `SViz` namespace.
(function (root) {
  const SPOT = 100, ASSUMED_IV = 0.60, R = 0.05;
  ...
  function bsPrice(S, K, T, sig, opt) {
  ...
  function bsGreeks(S, K, T, sig, opt) {
    if (T <= 0 || sig <= 0) {
  ...
  function strikeFromDelta(target, T, sig, opt) {
  ...
  function legPremium(leg, K) {
    if (leg.type === "perp") return SPOT;
    const T = Math.max(leg.dteTarget ?? 14, 1) / 365;
    return bsPrice(SPOT, K, T, ASSUMED_IV, leg.optionType ?? "CALL");
  }
  function legPayoffAt(leg, s, K, prem) {
  ...
  // ── text helpers (mirrors _specs.py) ────────────────────────────────
  function hrs(h) { return h % 24 === 0 && h >= 24 ? `${h / 24}d` : `${h}h`; }

  function gateLabel(mode, gmin, n) {
  ...
  root.SViz = {
    SPOT, ASSUMED_IV, R, REASON_COLORS,
    erf, nCdf, nPdf, bsPrice, bsGreeks, strikeFromDelta,
    legStrike, legPremium, legPayoffAt, legPayoffVec,
    legGreeksAtEntry, portfolioGreeks,
    hrs, gateLabel, legLabel, entryRows, exitRows, thesis, escapeHtml,
  };
})(window);
